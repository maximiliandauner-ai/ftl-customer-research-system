...
Every non-archived company with a non-disputed domain—including identities awaiting merge review—also receives a separate source-backed profile-enrichment run. The safe fetcher starts at the official HTTPS homepage, follows at most four same-domain imprint/about/company links, and extracts only explicit JSON-LD, legal-notice, address, employee-count, and bounded official-description evidence. Legal name, type, industry, headquarters, employee range, and description are filled when supported; unavailable facts remain visibly unknown. Immutable per-field observations retain their official URL, retrieval time, exact excerpt, hash, method, and confidence, while previously manual values are not overwritten. New source registrations and recurring known-endpoint scrapes queue this automatically with daily idempotency, and a weekly schedule refreshes profiles older than 30 days or produced by an older parser policy. Existing companies can be queued idempotently with:

```sh
docker compose --env-file .env -f compose.yaml -f compose.dev.yaml exec web \
  python manage.py backfill_company_profiles --limit 500
```
