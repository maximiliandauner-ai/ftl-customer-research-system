...
- Production backfills covered all 107 non-archived companies with eligible domains. All runs settled: 70 produced complete or partial source-backed profiles and 37 failed safely on identity, DNS, HTTP-status, or retry policy without applying unsupported values. New source registrations and recurring known-endpoint scrapes queue daily-idempotent enrichment directly; backfill and weekly refresh also include merge-review identities.
