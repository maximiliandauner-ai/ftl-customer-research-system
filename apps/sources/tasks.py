...
from apps.sources.services import RetryableFetchError, execute_source_fetch, mark_fetch_exhausted


@shared_task(  # type: ignore[untyped-decorator]
    bind=True,
    name="sources.fetch_public_source",
    ignore_result=True,
    acks_late=True,
...
def fetch_public_source_task(task: Any, envelope_data: dict[str, object]) -> None:
    ...
    try:
        ...
        if task.request.retries >= task.max_retries:
            mark_fetch_exhausted(pipeline_run_id=envelope.pipeline_run_id)
            return
